...
| 8 | [进阶专题：经典ML理论高地](./08-进阶专题-经典ML理论高地.md) | ⭐⭐⭐⭐ | 选读高阶：核方法统一视角（谱分析+表示定理）、Boosting 理论与统一数学账、深度学习何时赢、Conformal 保形预测、分布偏移与因果之桥 |
| 9 | [近邻与朴素贝叶斯](./09-近邻与朴素贝叶斯.md)（经典补充） | ⭐⭐ | 非参数与生成式两路线的极值：懒惰学习与 k/距离选择、维数灾难的推导与数字例；朴素贝叶斯的独立分解与拉普拉斯平滑（与第 2 章垃圾邮件算例互链） |
| 10 | [特征工程](./10-特征工程.md)（经典补充） | ⭐⭐ | 表格数据的分数第一来源：数值/类别/文本/时间/交叉五族、过滤/包裹/嵌入三路线；深潜目标编码泄漏与 K 折交叉拟合（07 页泄漏主题延伸） |
| 11 | [概率图模型](./11-概率图模型.md)（经典补充） | ⭐⭐⭐ | 把指数大的联合分布画成图：贝叶斯网/马尔可夫网/链，与第 11 章因果图划清语义边界；HMM 前向与维特比（穷举 O(Nᵀ)→DP O(N²T) 深潜） |

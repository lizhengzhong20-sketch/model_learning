...
> 📐 这套"倒着翻账本"为什么比"顺着翻"便宜一百万倍——局部导数表与两种模式的代价账，见下方「数学深潜：自动微分的机械规则」。
...
### 📐 数学深潜：自动微分的机械规则——局部导数、前向模式 vs 反向模式

**严格陈述。** 任何程序算出的标量 L 都能拆成一张**计算图**：节点 = 中间变量，边 = 初等运算。自动微分（Automatic Differentiation, AD）建立在两个事实上：

1. 每种初等运算只需知道自己的**局部导数**（一张小表查得）；
2. 链式法则在图上有两种求值顺序——**前向模式**（从输入出发顺着图走）与**反向模式**（从损失出发倒着走）。深度学习"损失是单个标量、参数却有百万个"的形状，决定了反向模式一次扫图即得全部偏导，前向模式则要扫百万遍——`backward()` 选择倒着走，理由全部在此。

**机械规则：局部导数表（每行附一行推导）。**

| 运算 | 局部导数 | 一行推导（差商定义 (f(x+h)−f(x))/h 取 h→0） |
|---|---|---|
| 加法 z = x + y | ∂z/∂x = 1，∂z/∂y = 1 | (x+h)+y − (x+y) = h，除以 h 恒为 1 |
| 乘法 z = x·y | ∂z/∂x = y，∂z/∂y = x | (x+h)y − xy = hy，除以 h 得 y |
| 平方 z = x² | dz/dx = 2x | (x+h)² − x² = 2xh + h²，除以 h 取极限留下 2x |
| 复合 z = f(g(x)) | dz/dx = f'(g(x))·g'(x) | 每个节点只报自己那一环的"传导率"，连乘即链式法则 |
| ReLU z = max(0, x) | x>0 时 1；x<0 时 0 | 两段都是直线，斜率分别是 1 和 0（折点处取次梯度） |

> 👉 **人话**：自动微分没有魔法——它只是把"每个零件的传导率"查表相乘。加法原样传递、乘法交换角色（对 x 求导得到 y）、复合逐环相乘。上一页手写的五环追责链，就是这张表连乘五次。

**迷你计算图（3 个运算节点，逐步标梯度流动）。** 取 $L = (x\cdot y + z)^2$，前向代入 x=3、y=4、z=1：

```mermaid
graph LR
    X["x = 3<br/>∂L/∂x = 104 ←"] --> U["① u = x·y = 12<br/>∂L/∂u = 26"]
    Y["y = 4<br/>∂L/∂y = 78 ←"] --> U
    U --> W["② w = u + z = 13<br/>∂L/∂w = 26"]
    Z["z = 1<br/>∂L/∂z = 26 ←"] --> W
    W --> L["③ L = w² = 169<br/>∂L/∂L = 1"]
```

反向扫一遍（从右往左，每站做一次"查表 × 收到的账"）：

| 反向步骤 | 查表得局部导数 | 收到的上游梯度 | 传出 = 乘积 |
|---|---|---|---|
| ③ → ② | dL/dw = 2w = 26（平方行） | 1 | 26 |
| ② → ① | ∂w/∂u = 1（加法行） | 26 | 26 |
| ② → z | ∂w/∂z = 1 | 26 | 26 |
| ① → x | ∂u/∂x = y = 4（乘法行） | 26 | **104** |
| ① → y | ∂u/∂y = x = 3 | 26 | **78** |

解析验证：∂L/∂x = 2(xy+z)·y = 2×13×4 = 104 ✓。注意"26"这个公共量**只算了一次**——反向模式一次扫全图的效率，正是把所有公共因子复用到极致的结果。

**前向 vs 反向：代价对比。**

| | 前向模式 | 反向模式 |
|---|---|---|
| 一次扫图得到 | 全部输出对**某一个输入**的偏导（Jacobian 的一列） | **标量损失对全部输入**的偏导（Jacobian 的一整行） |
| 拿全梯度要扫几次 | n 次（n = 参数个数，百万级） | **1 次**（外加一次前向） |
| 单次代价 | ≈ 前向计算本身 | ≈ 2~5 倍前向（Baur–Strassen 定理给的常数上界） |
| 内存 | 不必存中间值 | 必须存前向全部中间值（第 4 页显存账本里的"激活值"就是它） |

$$
\underbrace{\frac{\partial L}{\partial (x_1,\dots,x_n)}}_{1\times n\ \text{的扁行向量}} = \underbrace{v^{\mathsf{T}}}_{1\times m}\cdot\underbrace{J}_{m\times n}
$$

> 👉 **人话**：深度学习的 Jacobian 是"一行宽、百万列长"的扁条——反向模式从左端一次推过去整行到手；前向模式一列一列算，百万列就是百万遍。反过来，若输出很多、输入很少（科学计算求灵敏度矩阵的常见形状），前向模式反而便宜。**哪种模式好取决于 Jacobian 的形状，不是信仰。**

**反例与边界。**

1. 反向模式必须存所有中间值——大模型显存爆炸的元凶之一；梯度检查点（activation checkpointing，用重算换显存）就是针对这条代价的工程反制（第 4 页与本章进阶专题）；
2. 求高阶导数时"反向套反向"代价按图的平方增长，实践中算 Hessian-向量积常用"前向套反向"（forward-over-reverse）；
3. 自动微分只对**实际走过的可导路径**负责：if/for 走了哪条分支就只对哪条分支求导（第 5 页 tracing 导出的坑与此同源）；对离散决策"求梯度"本身没有意义。

> 👉 **人话总结**：自动微分 = 查表（局部导数）+ 链式法则的两种走法；损失只有一个而参数百万，`backward()` 倒着走一次比每个参数各走一次便宜一百万倍——这是 PyTorch"记账本"设计真正的数学底气。
...
框架设计的**真实取舍**：eager 模式是"为研究者优化的执行模型"——每个算子一次 Python 调度，用性能换生产力；`torch.compile` 则是"把 Python 的灵活性圈起来再卖掉"：tracing 捕获一段子图 → 用全图视角优化（融合、内存复用）→ 用 guard（守卫条件）保证输入变了就安全重捕。它不是静态图的简单回归，而是"eager 前端 + 编译后端"的杂交——这是过去十年 TF1（纯静态）→ PyTorch（纯动态）→ PyTorch 2.x（动前静后）这条螺旋的历史逻辑。

三个深问深答（研究生资格考试 / 高级面试级别）：

**深问一：torch.compile 为什么对大模型加速明显，对小模型反而可能变慢？**

答：收益有三个来源——算子融合（省访存与 kernel 启动）、消除 Python 级调度开销、更激进的内存规划。大模型算子大、中间激活多，融合省下的访存占比高；小模型则有两笔反向账：① **首次编译成本**（几十秒到分钟级）在快速实验迭代里反复发生（改一个形状、改一个控制流就触发重编译，"重编译风暴"是真实痛点）；② 运行期每次调用要过 guard 检查，小模型的算子本来就 launch-bound，guard 开销占比反而显眼。工程结论：**长跑的训练/推理服务开 compile，短平快的调试实验关掉**（经验参考）。

**深问二：no_grad 与 inference_mode 的真实区别是什么？**

答：差两层账本。`no_grad` 只是不建计算图，但张量的**版本计数（version counter）与视图追踪（view tracking）照做**——这套机制是 autograd 用来检测"原地操作改了旧值"的安全网；`inference_mode` 连安全网都拆了，产出"inference 张量"：之后不能再用于需要 autograd 的计算。大量小算子的纯推理里，免记账的累积节省可观。这也是"推理服务用 inference_mode、训练中间偷看数值用 no_grad/detach"这条经验背后的机制依据（见上文⑦的表）。

**深问三：动态图的真实代价是什么——为什么"边执行边建图"最终需要"先看全图再优化"来补？**

答：eager 模式下每个算子独立调度，框架在执行任何一行时都**看不到下一行**：没法跨算子融合（中间结果必须落显存再读回）、没法全局规划显存复用、没法做自动并行切分。全图可见恰恰是这三件事的前提——这正是 TF1 时代静态图换来的、被易用性掩盖的能力。`torch.compile` 的解法是折中：用 tracing/dynamo 捕获子图，编译期做全局优化，运行期用 guard 守住"捕获时的假设"。第 5 页 ONNX 导出（tracing）与本章[进阶专题](./06-进阶专题-大规模训练工程.md)的 MFU 性能工程，都建立在这套"图捕获"基础设施上。

## 🔬 SOTA 演进（2023–2026）

| 时间 | 工作 | 改进点 | 代价 | 适用 |
|------|------|--------|------|------|
| 2023 起 | **torch.compile**（PyTorch 2.x，dynamo 捕获 + Inductor 后端） | eager 写法不变，捕获子图后生成融合 kernel（Triton/C++），大模型训练与推理常见两位数百分比级的提速（经验参考） | 首次编译慢、动态形状/控制流触发重编译、报错栈变深 | 长跑任务默认开；快速调试关掉 |
| 2023–2026 | 静态化思想全面回归 | "先捕获再优化"成为共识：训练侧 compile、导出侧 torch.export（替代裸 tracing）、部署侧与第 5 页 ONNX 链路衔接 | 学习成本：要理解 guard、动态形状标注 | 新项目直接按 2.x 心智模型设计 |
| 2024–2025 | 分布式调试工具持续补齐（一句话级） | 如 NCCL 调试日志、分布式飞行记录仪（flight recorder）、`torch.distributed.breakpoint` 类断点工具——多卡 hang 从"print 大法"变成有工具可查 | 工具仍在快速演化，版本间行为有差异 | 大规模训练前先升级到近期版本再上多卡 |

一句话判断现状：**PyTorch 的"易用"没有变，但"快"的责任正从用户手写技巧（第 2 页的混合精度、第 4 页的并行）转移到编译器**——写代码时保持"图友好"（避免数据相关的控制流、显式标注动态维度），是 2023 年以后的新工程素养。

## ❓ 开放问题

1. **eager-first 与 compiled-first 的框架路线之争何时收敛？** 为什么难：科研要求"任何 Python 都能跑"，编译器要求"代码可推理"——两者数学上是矛盾的，折中方案的位置随硬件与编译技术漂移。
2. **编译器能否彻底吃掉手写 kernel？** 为什么难：Triton 类工具把门槛降了一个数量级，但 SOTA 性能仍依赖专家对寄存器/访存模式的微观调优，这类知识封闭在少数工程实践中，难以规则化进编译器。
3. **自动微分的模式单一化（reverse mode 一统训练）是必然吗？** 为什么难：forward mode 与基于隐函数定理的路线在二阶量、科学计算上有复杂度优势，但生态（教程、算子支持、硬件加速库）的路径依赖让迁移成本居高不下。
4. **多卡程序的非确定性 bug（hang、静默数值分叉）能否被系统性消灭？** 为什么难：根因散布在应用层、集合通信库、驱动与网络硬件之间，复现需要同规模集群——调试成本随卡数上涨，而开源社区没有等规模的复现环境。

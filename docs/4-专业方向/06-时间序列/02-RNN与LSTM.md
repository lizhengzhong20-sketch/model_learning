...
> 📐 上面的连乘直觉可以规范化成带范数的定理——上界的逐步推导、谱半径的精确分界与数值例，见下方「数学深潜：BPTT 的梯度范数界」。
...
> 📐 "主项近似单位阵"的完整版——直接项/间接项的精确分解、范数量级与百步连乘的账，见下方「数学深潜：∂c_t/∂c_{t−1} = diag(f_t)——细胞状态的直通公路」。
...
### 📐 数学深潜：BPTT 的梯度范数界——谱半径条件与"两种病"的严格化

② 的折叠推出了雅可比的形状与两种病的直觉，这一节把它规范化成带范数的定理：回传梯度被"相邻步传导矩阵的范数之积"从上方控制，条件落在 $W_h$ 的最大奇异值（谱半径的范数化身）上。

**严格陈述。** 朴素 RNN $h_{k+1} = \tanh(W_x x_{k+1} + W_h h_k + b)$，末端损失 L，记 $D_{k+1} = \mathrm{diag}(1 - h_{k+1}^2)$（tanh 的雅可比，对角元 ∈ (0, 1]）。则对任意 t < T：

$$
\left\| \frac{\partial L}{\partial h_t} \right\|_2 \;\le\; \left\| \frac{\partial L}{\partial h_T} \right\|_2 \cdot \prod_{k=t}^{T-1} \big\| D_{k+1} W_h^{\top} \big\|_2 \;\le\; \left\| \frac{\partial L}{\partial h_T} \right\|_2 \cdot \big(\sigma_{\max}(W_h)\big)^{T-t}
$$

（$\sigma_{\max}$ 为最大奇异值。）于是 $\sigma_{\max}(W_h) \le 1 - \delta$ 时梯度至多按 $(1-\delta)^{T-t}$ 指数衰减；训练早期（h ≈ 0 非饱和、D ≈ I）若 $\sigma_{\max}(W_h) > 1$，沿最大奇异方向的梯度分量按 $\sigma_{\max}^{\,T-t}$ 指数增长。

> 👉 **人话**：回传梯度像接力传递的钱，每过一步至多被折损/放大 $\sigma_{\max}(W_h)$ 倍——tanh 的对角阵（≤ 1）只会再折损。所以账全记在 $W_h$ 的最大奇异值头上：小于 1 是指数漏钱（消失），大于 1 是指数印钱（爆炸）。

**完整推导。**

<details>
<summary>🧮 展开推导：链式展开 → 范数次乘性 → 对角阵 ≤ 1 → 数值例与"爆炸可达"</summary>

**第一步：链式法则展开成积。** 复合函数逐层求导（② 已推）：

$$
\frac{\partial L}{\partial h_t} = \frac{\partial L}{\partial h_T}\cdot \prod_{k=t}^{T-1} \frac{\partial h_{k+1}}{\partial h_k}, \qquad \frac{\partial h_{k+1}}{\partial h_k} = D_{k+1}\, W_h^{\top}
$$

**第二步：范数次乘性（$\|AB\|_2 \le \|A\|_2\,\|B\|_2$）。** 对任意向量 v：$\|ABv\|_2 \le \|A\|_2\,\|Bv\|_2 \le \|A\|_2\,\|B\|_2\,\|v\|_2$；对单位向量取上确界即得。于是"乘积的范数 ≤ 范数的乘积"：

$$
\Big\| \prod_{k} D_{k+1} W_h^{\top} \Big\|_2 \;\le\; \prod_{k} \|D_{k+1}\|_2 \cdot \|W_h^{\top}\|_2
$$

**第三步：两个因子分别算。**

- 对角阵的谱范数 = 最大对角元绝对值：$\|D_{k+1}\|_2 = \max_i (1 - h_{k+1,i}^2) \le 1$（tanh 输出 ∈ (−1,1) ⇒ 对角元 ∈ (0,1]；**h 越接近 ±1（饱和）越小**）；
- $\|W_h^{\top}\|_2 = \|W_h\|_2 = \sigma_{\max}(W_h)$（转置不改奇异值）。

代入即得定理的上界。∎

**第四步：数值例（标量版，一切可见）。** 取 $W_h = \phi$（标量）、h 在非饱和区（$D \approx 1$）、$\|\partial L/\partial h_T\| = 1$，回传 100 步：

| $\sigma_{\max}$ | 界 $\phi^{100}$ | 100 步后的梯度量级 |
|---|---|---|
| 0.8 | $0.8^{100} \approx 2 \times 10^{-10}$ | 消失：学习信号归零（**无声病**） |
| 1.0 | 1 | 临界：勉强持平，tanh 一饱和就滑向消失 |
| 1.2 | $1.2^{100} \approx 8 \times 10^{7}$ | 爆炸：一步更新踢飞参数、loss 变 NaN（**响亮病**） |

**为什么"爆炸"不只是上界松**：训练早期 h ≈ 0 ⇒ D ≈ I，乘积 ≈ $(W_h^{\top})^{T-t}$；若 $\sigma_{\max} > 1$，取对应奇异向量方向 v，$\|(W_h^{\top})^{n} v\|$ 按 $\sigma_{\max}^{\,n}$ 增长——该方向的梯度分量真实指数放大。消失与爆炸分别是 $W_h$ 谱的两端的**真实行为**，不是放缩的 artifact。
...
```
 log₁₀‖∂L/∂h_t‖（随回传步数变化，示意）

  +8 ┤                                 ●  σ=1.2：上天（变 NaN 前最后一站）
     │                            ●
   0 ┤ ●━━━━━━━━━━━━━━━━━━━━━●         σ=1.0：勉强持平（饱和一出现就下滑）
     │                       ●
 −10 ┤                   ●             σ=0.8：一步折损两成，百步归零
     │              ●
     └───┬────┬────┬────┬────┬───► 回传步数 T−t
         0   25   50   75  100
 裁剪（③）把 +8 的尖峰削回阈值——但它拉不起 −10 的谷底：只治爆炸，不治消失。
```

**反例与边界。**

1. **范数条件是充分的、保守的**：$\sigma_{\max} < 1$ 保证消失，但 $\sigma_{\max} > 1$ 不必然爆炸——乘积 $(W^{\top})^n \to 0$ 的**精确**分界是谱半径 $\rho(W) < 1$（奇异值 ≥ 特征值模长，$\rho \le \sigma_{\max}$）；正交初始化（$\sigma_{\max} = 1$）正是卡在分界线上"既不消也不涨"的工程取法；
2. **tanh 的对角元只帮倒忙**：$\|D\| \le 1$ 说明激活函数**只压不涨**——爆炸的燃料永远来自 $W_h$，这就是"裁剪治爆炸、结构治消失"分工的数学出处；
3. **本定理对 LSTM 失效（好事）**：LSTM 的 c 通道传导含恒等项（下一深潜），连乘里出现 1 而非 $\sigma_{\max}$——"传导全靠矩阵乘"这一前提不成立，结论自然不再适用；
4. **梯度小 ≠ 严格学不到**：消失说的是上界趋零，微弱信号理论上仍在——但会被浮点下溢与学习率噪声吞没，工程上等价于学不到。

> 👉 **人话总结**：把 BPTT 的连乘逐项上界化，"能否长期记忆"就变成一个数的审判：$\sigma_{\max}(W_h)$——小于 1 无声地忘，大于 1 响亮地炸；两副药（裁剪管上限、结构管下限）在这条不等式的两端各就各位。

### 📐 数学深潜：∂c_t/∂c_{t−1} = diag(f_t)——细胞状态的直通公路

④ 的折叠用一句话带过"主项是近似单位阵"。这一节把这句话补成完整的账：直接项恰好是 diag(f_t)，间接项长什么样、多小、什么时候可以忽略，以及这条公路与 ② 连乘困局的正面对比。

**严格陈述。** LSTM 的记忆更新 $c_t = f_t \odot c_{t-1} + i_t \odot \tilde{c}_t$，其中三道门与候选都由 $[h_{t-1}, x_t]$ 算出、$h_{t-1} = o_{t-1} \odot \tanh(c_{t-1})$。则完整雅可比分解为**直接项 + 间接项**：

$$
\frac{\partial c_t}{\partial c_{t-1}} = \underbrace{\mathrm{diag}(f_t)}_{\text{直通公路（加法通道）}} + \underbrace{R_t}_{\text{经 } h_{t-1} \text{ 的绕行小路}}, \qquad R_t = \frac{\partial c_t}{\partial h_{t-1}}\cdot \mathrm{diag}\big(o_{t-1} \odot (1 - \tanh^2(c_{t-1}))\big)
$$

$\|R_t\|$ 含三道门的 sigmoid 导数（≤ 1/4）与 tanh 导数（≤ 1）的连乘，量级显著小于直接项；$f_t \approx 1$ 时 $\partial c_t/\partial c_{t-1} \approx I$，梯度**原样过站**。

> 👉 **人话**：记忆通道对"昨天的自己"求导，头一项就是遗忘门本身——门全开（f=1）时导数恰是单位阵，梯度原封不动往前传；另有几条从隐藏状态绕行的小路，又窄（sigmoid 导数压到 1/4 以下）又绕，平时构不成主干道。

**完整推导。**

<details>
<summary>🧮 展开推导：乘积法则拆两项 → 间接项两段逐一算 → 百步连乘的账</summary>

**第一步：认出 $c_{t-1}$ 出现了两次。** 明面一次（$f_t \odot c_{t-1}$），暗地一次（$f_t, i_t, \tilde{c}_t$ 都经 $h_{t-1}$ 依赖 $c_{t-1}$）。按乘积法则（和的导数 = 导数之和；"谁求导谁当常数"）：

$$
\frac{\partial c_t}{\partial c_{t-1}} = \underbrace{\mathrm{diag}(f_t)}_{\text{把门当常数}} + \underbrace{\frac{\partial \big(f_t \odot c_{t-1} + i_t \odot \tilde{c}_t\big)}{\partial h_{t-1}} \cdot \frac{\partial h_{t-1}}{\partial c_{t-1}}}_{\text{门随 } c_{t-1} \text{ 变化的部分}}
$$

**直接项**：固定 $f_t$ 后 $\partial (f_t \odot c_{t-1})/\partial c_{t-1}$ = 逐维乘法求导 = $\mathrm{diag}(f_t)$。∎（恒等式的本体就是这一行。）

**第二步：间接项的段一 $\partial h_{t-1}/\partial c_{t-1}$。** 由 $h_{t-1} = o_{t-1} \odot \tanh(c_{t-1})$，且 $o_{t-1}$ 在上一时刻已定（对 $c_{t-1}$ 是常数）：

$$
\frac{\partial h_{t-1}}{\partial c_{t-1}} = \mathrm{diag}\big(o_{t-1}\big)\cdot\mathrm{diag}\big(1 - \tanh^2(c_{t-1})\big)
$$

（逐维链式：每维 = 输出门开度 × tanh 导数，两个因子都 ≤ 1。）

**第三步：间接项的段二 $\partial c_t/\partial h_{t-1}$。** 三门一候选各对 $h_{t-1}$ 求雅可比（标准结果：$\partial \sigma/\partial z = \sigma(1-\sigma) \le 1/4$；$\tanh' \le 1$；预激活对 $h_{t-1}$ 的雅可比是权重转置）：

$$
\frac{\partial c_t}{\partial h_{t-1}} = \mathrm{diag}(c_{t-1})\, W_f^{\top} \mathrm{diag}\big(\sigma'(z_f)\big) + \mathrm{diag}(\tilde{c}_t)\, W_i^{\top} \mathrm{diag}\big(\sigma'(z_i)\big) + \mathrm{diag}(i_t)\, W_c^{\top} \mathrm{diag}\big(\tanh'(z_c)\big)
$$

（每一项 = "对侧取值 × 权重转置 × 激活导数"；全部激活导数对角元 ≤ 1，sigmoid 的 ≤ 1/4。）

**第四步：量级对比与连乘的账。** 间接项每条通道都要过激活导数（≤ 1/4 或 ≤ 1）再乘权重——**结构上就是 ② 朴素 RNN 式的"压缩因子 × 矩阵"连乘**，随步数衰减；直接项没有任何激活导数，就是 f_t 本身：

$$
\text{朴素 RNN 主干：} \prod_k D_{k+1} W_h^{\top} \;\lesssim\; \sigma_{\max}^{T} \qquad\qquad \text{LSTM 主干道：} \prod_{k} \mathrm{diag}(f_k) = \mathrm{diag}\Big(\prod_k f_k\Big)
$$

（对角阵连乘 = 逐维连乘：第 j 维的传导系数 = $\prod_k f_{k,j}$——**连乘的每个因子由网络通过门自己控制**，想传多久就把门开多久。）

**第五步：百步的账（标量化看第 j 维）。** f 恒 1 → $1^{100} = 1$（梯度无损）；f 恒 0.99 → $0.99^{100} \approx 0.37$（缓慢衰减，百步仍剩三分之一量级）；f 恒 0.9 → $0.9^{100} \approx 2.6\times 10^{-5}$（照样消失！）。而 sigmoid 保证 f ∈ (0, 1)：**主干道永不爆炸，但可以自己选择遗忘**。
...
```
             ┌────────────── c_{t−1} ──────────────┐
             │               │（明路）              │（暗路）
             │         f_t ⊙ ·（加法）         h_{t−1} = o ⊙ tanh(c_{t−1})
             │               │                     │
             │               │ + i_t ⊙ c̃_t       三道门经 h_{t−1} 绕行
             ▼               ▼                     ▼
   梯度回传：diag(f_t) 原样过站      R_t：×σ′(≤1/4)×W×tanh′(≤1) 层层折损

   主干道 = 门控的"可调恒等"：f=1 时是单位阵（CEC：常数误差传送带）
   小  路 = 朴素 RNN 式连乘（② 的病在小路上原样保留、只是被压小）
```

**反例与边界。**

1. **f 持续小于 1，照样指数消失**：$0.9^{100} \approx 10^{-5}$——LSTM 没有"解决"长依赖，是把"要不要衰减"从**权重的谱**交到**门的手里**（深度视角 CEC 的定量版）；训练出的遗忘门长期偏低 = 模型学会了健忘（⑤ 门统计诊断的理论依据）；
2. **f 恒 ≤ 1 ⇒ 主干道不爆炸**：爆炸只剩间接项一条小路，量级小且可被裁剪兜底——这就是"LSTM 训练稳得多、但梯度裁剪照开"的账本；
3. **间接项不是零**：长序列上仍可能积累（尤其 $W_f, W_i$ 奇异值大时）；**peephole 变体**（门直接读 c）会把 c 的依赖直接写进门里，间接项结构改变——恒等式陈述需相应改写；
4. **"≈ I"是逐步的、不是全程的**：全程传导 = 各步 diag(f) 之积（逐维），任何一步某维的门关死，该维的信息与梯度就断在那一步——公路上有收费站，每站开度都是学出来的。

> 👉 **人话总结**：对记忆求导，第一项就是遗忘门——这条加法直通道让梯度不必再蹭权重的奇异值过活：门全开则无损通行，门半开则可控衰减，门恒关照样断流；LSTM 的全部魔法，是把连乘的每个因子从固定权重换成了**可学习的阀门**。
...
**"传送带"才是 LSTM 论文的原始洞察**。本页推导的"记忆高速公路"在原始论文里有个名字：**常数误差传送带（Constant Error Carousel, CEC）**——误差沿 c_t 的加法通道不衰减地流动。它把"长依赖学不动"从**结构必然**降级为**学习问题**（门能不能学会该开该关）。这个视角的推论很重要：LSTM 从未"解决"长依赖，只是把障碍从"梯度物理"搬到了"优化与数据"——所以几百步以上的依赖依然吃力。

**记不住是可训练性问题，不是表达能力问题**。经典可计算性研究表明，循环网络（配以适当容量与权重）在理论上足以模拟任意有限状态计算——表达能力从未短缺，缺的是让梯度找到那组权重的可靠路径。这解释了本页所有努力的定位：门控、裁剪、跳接全是**优化侧**的手术，模型本身的"上限"一直很高。深度学习史反复上演同一幕：瓶颈先出现在优化，再出现在容量。

**定长状态的信息论天花板**。一个 d 维状态向量最多携带有限信息；要用它装下任意长度的历史，信息必然被丢弃——这不是 LSTM 的缺陷，是"循环 + 定长状态"**范式级**的天花板。出路只有两条：把历史摊开不压缩（注意力的本质，03 页），或让状态的取舍更聪明（输入依赖的选择性——Mamba 的"选择机制"正是这条路，见本章[进阶专题：序列建模前沿](./04-进阶专题-序列建模前沿.md)）。RNN → 注意力 → SSM 的演进，本质是围绕"状态该怎么装历史"的三次回答。

**批判视角**：①门控是工程胜利，不是理论必然——"为什么恰好三道门/两道门就够"主要靠事后解释；② RNN 的衰落一半输在精度，一半输在**不可并行训练**的工程账——逐时刻递推与 GPU 的批量矩阵乘天然不友好。这个教训后来成了架构设计的一等公民约束：今日评估任何新序列架构，"能不能吃满 GPU"与"精度多高"同权重。

## 🔬 SOTA 演进

| 方向 | 一句话 |
|---|---|
| **Mamba 类架构（谁还在用 RNN）** | 选择性状态空间模型（Mamba）以"RNN 式线性推理 + 可并行训练"继承了循环家族的衣钵——今天 LSTM/GRU 的主场收缩为小数据、边缘设备与流式逐点场景，超长序列建模则多了 Mamba 一条新路；谱系与边界详见[进阶专题：序列建模前沿](./04-进阶专题-序列建模前沿.md)。 |

## ❓ 开放问题

- **定长状态 vs 无限历史，有根本解吗**：信息论视角的答案倾向"没有"——除非状态维数随信息量增长，或外置记忆（注意力即"全量外置"的极端形式）。"压缩历史"与"保留历史"的张力会长期存在。
- **循环架构的复兴能走多远**：并行扫描把"递推不可训练"的诅咒解开后，SSM 一系重新参赛；但在需要精确逐位检索的任务上仍逊于注意力（专题页边界表）——循环与注意力的混合会否是长期形态，尚在演化中。
- **门值的解释边界**：门信号热图"看起来合理"（遗忘门在段落切换时关闭）与"因果上正确"是两回事——可视化是诊断线索，不是证明。

...
| 4 | [进阶专题：RL 全景扩展](./04-进阶专题-RL全景扩展.md) | ⭐⭐⭐⭐ | RL 地图剩下的四块大陆：基于模型的 RL（世界模型三件套、Dreamer 与 MuZero 的路线分歧）、MCTS 与 AlphaGo/AlphaZero/MuZero 演进链（UCB 深潜、逐代拆掉人类先验）、模仿学习（行为克隆的 O(εT²) 误差复利、DAgger、与 RLHF 的亲缘）、多智能体一览（博弈形态、自我博弈、CTDE） |

...
本章只有两节正文，但概念密度不低：第 1 节是纯直觉 + 一张小小的 Q 表，建议拿纸笔跟着手算（数字都不难，贝尔曼推导放折叠框里可分两次消化）；第 2 节进入神经网络时代，重点不是推公式，而是理解三个病根与三个药方一一对应的关系，clip 推导的分段讨论值得慢读。代码环节需要 `pip install gymnasium numpy`（[环境搭建](../../1-起步准备/01-环境与工具/01-开发环境搭建.md)见第 1 章），两段代码都附"改什么参数、看什么变化"的实验清单，值得逐条跑。

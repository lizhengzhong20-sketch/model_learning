...
> 世界不是一排排表格，而是一张张网：社交关系、分子结构、路网、用户-物品交互、知识图谱、欺诈团伙……这些数据的重点不在单个点，而在**点和点怎么连**。CNN 和 RNN 都吃不下这种数据——图没有固定顺序、大小可变、每个节点的邻居数都不一样。图神经网络（GNN, Graph Neural Network）用一个朴素到惊人的思想解决它：**每个节点反复"听邻居的、更新自己的"**，几轮之后，每个节点就携带着它周围结构的信息了。本章一页认识图数据（含真实图的四个脾气：幂律、断裂、同构难题、同质异质），一页吃透核心模型（消息传递、过平滑与邻居爆炸两面墙），一页潜入深水区（WL 表达力理论、GraphTransformer 与混合架构、大规模训练工程、时序图），最后一页看真实生产。
...
graph TD
    ...
    B --> D["03 进阶专题：GNN 深水区<br/>WL 表达力理论 / GraphTransformer 与混合架构<br/>图分区与采样 / 时序图 / 图基础模型之问"]
    D --> C["99 生产实战<br/>风控与推荐中的图方案<br/>实时计算 / 推理服务化 / 图体检"]
```
...
| 3 | [进阶专题：GNN 深水区](./03-进阶专题-GNN深水区.md) | ⭐⭐⭐⭐ | 理论与工程的天花板与账单：WL 上界与分不开的双胞胎图、GraphTransformer 的位置编码与"局部+全局"混合架构、图分区与采样三策略、边界信息丢失、时序图与动态更新；两个开放问题（过平滑 vs 表达力、图基础模型） |
| 4 | [进阶专题：GraphRAG 与知识图谱](./04-进阶专题-GraphRAG与知识图谱.md) | ⭐⭐⭐⭐ | 图当数据结构用：TransE 的平移几何（h+r≈t）与 1-N 关系的崩塌、链接预测的负采样与 MRR/Hit@K（采样评估的坑）、GraphRAG 治朴素 RAG 三盲区（多跳/全局/结构化事实）、企业知识图谱的构建成本账单 |

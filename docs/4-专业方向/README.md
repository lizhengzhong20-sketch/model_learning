...
> **七个方向，七种能力：看见、预知、读写、匹配、关系、决策、归因。** 它们大致平行、按兴趣或需要选读一两个即可（注意：视觉前沿、现代时序、推荐与图的多处内容会用到第 7 章的 Transformer——各页顶部的"前置知识"会提醒你）；排序本身就是一条认知递进线——从感知世界，到理解语言，到连接人与物，到做出决策，再到追问原因。

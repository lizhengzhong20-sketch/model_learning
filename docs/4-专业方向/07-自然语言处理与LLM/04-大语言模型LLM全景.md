...
### 📐 数学深潜：温度的数学——能级分布与两个极限

**严格陈述**。带温度的 softmax（把 logits $z$ 想成"负能量"）为：

$$
P_T(i) = \frac{\exp(z_i / T)}{\sum_{j} \exp(z_j / T)}
$$

**命题一**（$T \to 0^+$）：若最大 logit 唯一（$z_{i^*} = M > z_j,\ \forall j \neq i^*$），则 $P_T \to$ one-hot（全部概率落在 $i^*$ 上）；若最大值有 $k$ 个并列，则 $P_T \to$ 在并列集上均匀（各 $1/k$）。**命题二**（$T \to \infty$）：$P_T(i) \to 1/V$ 对所有 $i$ 成立，且一阶修正为 $P_T(i) \approx \frac{1}{V}\big(1 + \frac{z_i - \bar z}{T}\big)$。**引理**（两个极限的钥匙）：任意两候选的几率比只被温度按比例缩放：

$$
\frac{P_T(i)}{P_T(j)} = \exp\!\Big(\frac{z_i - z_j}{T}\Big)
\qquad\text{即} \log\frac{P_T(i)}{P_T(j)} = \frac{1}{T}\log\frac{P_1(i)}{P_1(j)}
$$

> 👉 **人话**：温度不改变候选的**排名**，只改变差距的**陡峭程度**——对数几率被除以 $T$。这与统计力学的玻尔兹曼（Boltzmann）分布 $P(E_i) \propto e^{-E_i/T}$ 完全同构：logits 是能级（取负），温度是热噪声的大小；低温系统趴在基态（argmax），高温系统所有能级均匀 populated（均匀分布）。采样温度就是给"词表这个物理系统"调温度。

<details>
<summary>🧮 展开完整证明：两个极限，一步不跳</summary>

**引理（几率比）**。直接写定义，分母相消：

$$
\frac{P_T(i)}{P_T(j)} = \frac{e^{z_i/T}\big/\sum_k e^{z_k/T}}{e^{z_j/T}\big/\sum_k e^{z_k/T}} = \frac{e^{z_i/T}}{e^{z_j/T}} = e^{(z_i - z_j)/T}
$$

$T$ 只以 $1/T$ 进入指数——分数被指数级放大（$T<1$）或压平（$T>1$），排名不变。

**命题一（$T \to 0^+$：退化为 argmax）。** 取 $M = \max_j z_j$，分子分母同乘 $e^{-M/T}$（不改变比值、避免数值溢出）：

$$
P_T(i) = \frac{e^{(z_i - M)/T}}{\sum_{j} e^{(z_j - M)/T}}
$$

逐项看极限：$z_i = M$ 的项，指数为 $e^0 = 1$；$z_j < M$ 的项，$z_j - M < 0$ 是固定负数，除以 $T\to 0^+$ 后趋于 $-\infty$，故 $e^{(z_j-M)/T} \to 0$。于是分母 $\to$ 并列最大值的个数 $k$，得

$$
P_T(i) \to \begin{cases} 1/k, & z_i = M \\ 0, & z_i < M \end{cases}
$$

唯一最大值（$k=1$）时即 one-hot——**贪心解码是零温极限**，这就是"T≈0 等价 argmax"的严格含义（并列时要靠采样器打破平局，见反例）。

**命题二（$T \to \infty$：趋近均匀）。** 对小的 $1/T$ 做 Taylor 展开 $e^{x} = 1 + x + O(x^2)$，取 $x = z_i/T$：

$$
e^{z_i/T} = 1 + \frac{z_i}{T} + O(T^{-2})
\qquad\Longrightarrow\qquad
\sum_j e^{z_j/T} = V + \frac{V\bar z}{T} + O(T^{-2})\quad(\bar z = \tfrac{1}{V}\textstyle\sum_j z_j)
$$

代入做除法（分子分母同除 $V$）：

$$
P_T(i) = \frac{1 + z_i/T + O(T^{-2})}{V\big(1 + \bar z/T + O(T^{-2})\big)} = \frac{1}{V}\Big(1 + \frac{z_i - \bar z}{T}\Big) + O(T^{-2}) \;\xrightarrow[T\to\infty]{}\; \frac{1}{V}
$$

一阶修正项把"怎么逼近均匀"也讲清了：**高出均值多少，就比 $1/V$ 暂时多占一点**，超出量随 $1/T$ 线性消退。

**数值验算**（本页"今天天气真"的表）：$z$ 比例对应 $P_1 = [0.60, 0.20, 0.10, 0.05, 0.05]$，前两名几率比 $3{:}1$。$T=0.5$ 时几率比 $= e^{2\ln 3} = 9{:}1$（富者愈富）；$T=2$ 时 $= \sqrt{3}{:}1 \approx 1.73{:}1$（均贫富）——与表中 0.87 vs 0.10、0.39 vs 0.23 的走势一致。

</details>

**几何/概率解释**（配图）：把候选想成能级、概率想成粒子数，温度控制粒子往低处（高分）集中的程度。

```text
能级图（logits 取负 = 能量，分数最高的词在最低能级）：

 能级 ↑        T→0（基态独占）        T 适中             T→∞（各层均摊）
      │  e3 ┤ ·                      · ·                 · · · · ·
      │  e2 ┤ · ·                    · · ·               · · · · ·
      │  e1 ┤ ·····                  · · · ·             · · · · ·
      │  e0 ┤ ██████████████  ★      ███████ ··          ███ ·····
      └───────────────────────────────────────────────────────────
              argmax（贪心）        正常采样             均匀（纯随机）

横条 = 各候选分到的概率质量；温度 = 把粒子从基态"烤"上去的能量。
```

**反例与边界**：

- **并列最大值**：$T \to 0$ 的极限是"并列集上均匀"，不是自动选第一个——工程里 greedy 与"零温采样"在并列时行为可能不同，而真实浮点 logits 几乎不严格并列，实践影响小但概念上要分清；
- **数值上不能真除以小 T**：$e^{z/T}$ 会溢出，实现必须用"减最大值"的稳定形式（本页证明里那步同乘 $e^{-M/T}$ 就是它的原型）；
- **温度治不了"高分错误"**：温度只重分配给定 logits 的概率，不改变哪个候选排第一——模型把错词排在第一时，降温只是让错误更坚定（幻觉的"斩钉截铁"正与此相关，见上文幻觉三来源）；
- **与 top-k/top-p 的分工**：温度是"连续调陡峭"，截断是"硬性删候选"——前者不删长尾、后者不改形状，二者正交所以常配合用。

> 👉 **一句人话**：温度就是把"对数几率表"整体除以一个数再 softmax——除得越小赢家通吃（贪心），除得越大天下大同（乱抽）；它能调"敢不敢冒险"，但永远治不了"押错宝"。
...
**真实取舍一：Chinchilla 配比是"训练算力最优"，不是"全生命周期最优"。** 20 token/参数 只优化一次性训练成本；一旦模型要服务海量请求，**推理成本随服役期累积**，最优解就变成"更小的参数 × 更多的数据"（over-training 小模型）。公开数字：Llama 3 8B 用了 15T token 训练，$D/N \approx 15000/8 \approx 1875$，是 Chinchilla 配比的近百倍——这不是违背 Scaling Law，而是把优化目标从"训练最优"换成了"推理最优"。**同一条幂律，算谁的账，答案不同。**

**真实取舍二：Scaling Law 能外推 loss，不能外推"能力上线"。** 损失曲线平滑可预测，是万卡集群规划的底气；但"loss 再降 0.05 对应哪些产品能力解锁"没有可靠映射（见深问 1）。工程纪律：**容量按 loss 规划，产品承诺绝不下沉到 loss 外推**——"等规模到了能力自然出现"是被涌现争议反复打脸的乐观。

**真实取舍三：压缩视角有解释力，没有操作性。** "预测下一词 = 压缩文本"统一了大量现象（为什么有语法、为什么有事实），甚至"语言建模即压缩"被推广成通用智能度量；但"压缩率 = 智能程度"始终缺少可操作定义——两个压缩率相同的模型，能力可能天差地别。把压缩当**透镜**有价值，当**指挥棒**会跑偏。

<details>
<summary>🧮 高级深问 3 则：loss 与能力的错位、数据墙、token 效率的不公平</summary>

**问 1：为什么 loss 只降了一点，能力却差很多（或反之）？** 交叉熵是对**全部 token 的平均**，而关键能力（一道多步推理的正确性）集中在少数"决定性 token"上：整段生成中 99% 的 token 是模板化衔接（谁都会），1% 是推理转折点（分高下）。平均损失对这 1% 极不敏感——loss 是温度计，不是天气预报；评估能力必须回到任务级指标，这正是[进阶专题](./07-进阶专题-LLM训练与推理全栈.md)评估学一节的出发点。

**问 2：数据墙（Data Wall）是真的吗？** 高质量人写文本的存量有限，而主流开源模型公开训练量已到 15T token 级——按现有增速，"高质量网页级文本"被反复消耗的时间表被广泛讨论。出路三条：合成数据、多模态语料、更精细的数据课程。风险也有实证：纯合成数据递归训练会出现**模型坍缩（Model Collapse）**——分布尾部逐代丢失、输出趋于同质；掺入足够比例的真实数据可显著缓解，但"安全配比"没有定量理论（开放）。

**问 3：中文的 token 效率劣势是工程问题还是结构问题？** 同一内容中文消耗更多 token（上文 tiktokenizer 实验可见），意味着**同算力预算下中文语料的"有效信息摄入"更低**——这是一种结构性吃亏。缓解手段在 tokenizer 工程（扩中文词表、整词入表，见[进阶专题](./07-进阶专题-LLM训练与推理全栈.md)数据工程一节）；但"字节级公平的 tokenizer 是否存在/值得"没有共识——它同时牵动模型能力、计费与生态锁定。

</details>

## 🔬 SOTA 演进（2023–2026）

- **小模型能力提升路径**：2023 年前"小模型注定平庸"的共识被推翻，两条公开路线立功——**高质量数据**（"教科书级"精选语料：少量高密度、结构清晰、错误率低的数据，把同等参数的模型推到从前大一级参数的水平）与**蒸馏（Distillation）**（让小模型学强模型的输出分布与推理轨迹，把"能力"而不只是"知识"压进小参数；注意蒸馏的版权与"模型谱系污染"争议仍在发酵，属开放问题）。
- **MoE 开源化**：开源 MoE 模型在 2024 年起集中涌现（总参数大、激活参数小，[03 页](./03-Transformer架构细节与变体.md)的账本：算力按激活、显存按总参数）——"大而不贵"从闭源专属变成开源标配路线；代价是部署门槛从"算力贵"变为"显存+工程复杂度贵"。

## ❓ 开放问题

1. **涌现有没有统一的"触发规模"？** 度量假象说与真实门槛说并存（正文已展开）；即便接受"任务级跳变真实存在"，不同能力是否在同一条规模轴上先后解锁、能否提前预测，均无定论。
2. **合成数据的长期效应**：模型坍缩在什么配比、多少代递归后失控，缺乏定量理论；"用模型 A 的数据训模型 B"的谱系追踪与许可问题，工程与法务都还没准备好。
3. **Scaling 的下一维**：数据受限后，算力的边际投向从"更大的模型"转向"推理时更多计算"（test-time compute，[进阶专题](./07-进阶专题-LLM训练与推理全栈.md)开放问题一节）与持续学习——训练算力与推理算力的最优分配，是 2025 年后 scaling 研究的新主线。

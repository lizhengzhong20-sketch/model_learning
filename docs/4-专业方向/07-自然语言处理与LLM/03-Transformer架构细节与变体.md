...
### 📐 数学深潜：RoPE 内积恒等式——q_mᵀk_n 只依赖 (m−n)

**严格陈述**。把 $d$ 维 Q/K 向量按相邻维配对切成 $d/2$ 个二维平面，第 $i$ 对平面配置角频率 $\theta_i = 10000^{-2i/d}$。记 $R_m \in \mathbb{R}^{d\times d}$ 为分块对角旋转矩阵（第 $i$ 块是平面 $i$ 上角度 $m\theta_i$ 的二维旋转）。**命题**：对任意位置 $m, n$ 与任意向量 $\mathbf{q}, \mathbf{k}$，

$$
\langle R_m\,\mathbf{q},\; R_n\,\mathbf{k} \rangle = \langle \mathbf{q},\; R_{n-m}\,\mathbf{k} \rangle
$$

即打分**只通过相对位置 $(n-m)$ 进入内积**，绝对位置 $m, n$ 本身被旋转抵消——这是 RoPE 一切性质（相对性、外推性）的唯一数学源头。

> 👉 **人话**：两只指针各转各的角度，读数只看**夹角差**——查询转了"第 100 位"的角度、键转了"第 95 位"的角度，内积里剩下的只有"差 5 位"。位置信息不是贴上去的门牌，是揉进角度差的几何。

<details>
<summary>🧮 展开完整证明：两条旋转矩阵性质 + 一次转置（不跳步）</summary>

**性质一：转置 = 反向旋转，$R(\alpha)^{\top} = R(-\alpha)$。** 写出二维旋转矩阵（约定逆时针为正）：

$$
R(\alpha) = \begin{bmatrix} \cos\alpha & -\sin\alpha \\ \sin\alpha & \cos\alpha \end{bmatrix}
\;\Longrightarrow\;
R(\alpha)^{\top} = \begin{bmatrix} \cos\alpha & \sin\alpha \\ -\sin\alpha & \cos\alpha \end{bmatrix} = R(-\alpha)
$$

（转置把非对角元的 $\mp\sin\alpha$ 互换符号，恰是角度取负。）

**性质二：旋转可加，$R(\alpha)R(\beta) = R(\alpha+\beta)$。** 老老实实做矩阵乘法：

$$
R(\alpha)R(\beta) = \begin{bmatrix} \cos\alpha\cos\beta - \sin\alpha\sin\beta & -\cos\alpha\sin\beta - \sin\alpha\cos\beta \\ \sin\alpha\cos\beta + \cos\alpha\sin\beta & -\sin\alpha\sin\beta + \cos\alpha\cos\beta \end{bmatrix}
$$

用两角和公式 $\cos(\alpha{+}\beta) = \cos\alpha\cos\beta - \sin\alpha\sin\beta$、$\sin(\alpha{+}\beta) = \sin\alpha\cos\beta + \cos\alpha\sin\beta$ 逐块替换：

$$
R(\alpha)R(\beta) = \begin{bmatrix} \cos(\alpha+\beta) & -\sin(\alpha+\beta) \\ \sin(\alpha+\beta) & \cos(\alpha+\beta) \end{bmatrix} = R(\alpha+\beta)
$$

**主证明（单一平面上）**。设该平面频率为 $\theta$，查询在位置 $m$、键在位置 $n$，各自被旋转 $m\theta$ 与 $n\theta$。用转置把内积写成矩阵乘积，再连用两条性质：

$$
\langle R(m\theta)\,\mathbf{q},\; R(n\theta)\,\mathbf{k} \rangle
= \mathbf{q}^{\top} R(m\theta)^{\top} R(n\theta)\,\mathbf{k}
= \mathbf{q}^{\top} R(-m\theta)\,R(n\theta)\,\mathbf{k}
= \mathbf{q}^{\top} R\big((n-m)\theta\big)\,\mathbf{k}
$$

$m, n$ 的绝对值完全消失，只剩差 $(n-m)$。

**推广到 $d$ 维**。$R_m$ 是分块对角阵，各平面独立执行上式（第 $i$ 平面用 $\theta_i$）；全向量内积是各平面内积之和：

$$
\langle R_m\mathbf{q}, R_n\mathbf{k}\rangle = \sum_{i=1}^{d/2} \mathbf{q}_i^{\top} R\big((n{-}m)\theta_i\big)\mathbf{k}_i = \langle \mathbf{q}, R_{n-m}\mathbf{k}\rangle
$$

多分辨率频率 $\{\theta_i\}$（秒针到千年历，见 [02 页](./02-Transformer与注意力机制.md)）保证不同的 $(n-m)$ 在至少部分平面上产生不同的旋转组合——相对位置可分。证毕。

**一个边界提醒**：证明里 $\sqrt{d_k}$ 缩放、softmax 都是外层装饰，恒等式只关于内积本身——所以它对任何单调变换打分都成立，不受注意力实现细节影响。

</details>

**几何解释**（配图）：每个维度对是一根指针，位置是累计转角；内积只测"指针夹角差"。

```text
平面 i（频率 θi）上，位置 m 与 n 的两根指针：
...
内积 qᵀk = |q||k|·cos(夹角)，夹角 = mθi − nθi = (m−n)θi
—— 两个绝对角度谁大谁小无所谓，只有差进入打分。
d/2 根转速不同的指针合起来 → 任意相对距离 (m−n) 都有独特"指纹"。
```

**反例与边界**：

- **恒等式只对"旋转"成立**：正弦位置编码（把位置**加**到向量上）得到的是"平移可由线性变换表达"的近似便利（[02 页](./02-Transformer与注意力机制.md)折叠框），不是本页这种严格恒等；换成任意非正交的位置变换，$(n-m)$ 就不再分离；
- **外推边界在"角度分布"而非公式**：恒等式对任意 $m, n$ 成立，但训练只见过的角度组合有限——超大 $(n-m)$ 对应的旋转组合是分布外输入，打分会失真，这正是位置插值/NTK 补丁的对象（本页上文）；
- **只作用于 Q/K**：旋转进不了 V——输出几何（值向量的凸组合）不受位置影响，位置信息只参与"看谁"，不参与"搬什么"。

> 👉 **一句人话**：RoPE 的全部魔法就一行——旋转矩阵转置等于反向、相乘等于角度相加，于是内积里两个绝对位置自动抵消、只剩相对差；剩下的（外推、插值）都是这句线性代数的工程余波。

### 📐 数学深潜：KV Cache 显存公式——四个因子从哪来（含 MHA/GQA 实算）

**严格陈述**。对自回归推理，每生成一个新 token 不必重算历史——历史各位置的 K/V 可以缓存。缓存显存总量为：

$$
\text{KV Cache 字节} = \underbrace{2}_{\text{K 与 V 两份}} \times \underbrace{L}_{\text{层数}} \times \underbrace{n_{\text{kv}} \cdot d_{\text{head}}}_{\text{每层 KV 总维 } d_{\text{kv}}} \times \underbrace{b}_{\text{每参数字节数}} \times \ell \times B
$$

（$\ell$ 为序列长度，$B$ 为并发 batch）。MHA 时 $n_{\text{kv}} = $ 头数、$d_{\text{kv}} = d_{\text{model}}$；GQA/MQA 时只有 $n_{\text{kv}}$ 缩小——**公式形状不变，这一个因子就是全部瘦身空间**。

> 👉 **人话**：缓存 = "层数 × 长度 × 每层 KV 维度 × 精度"四因子连乘再乘两份（K、V）——上下文翻倍缓存翻倍、batch 翻倍缓存翻倍，长上下文服务的入场费就写在这一行里。

<details>
<summary>🧮 展开完整推导：逐因子数出来（不跳步）</summary>

**第 1 步：数一个 token 在一层头上占多少。** 生成到第 $\ell$ 个位置时，注意力需要每个历史位置 $j \le \ell$ 的 $\mathbf{k}_j$（参与打分）与 $\mathbf{v}_j$（参与加权）。每个 $\mathbf{k}_j$、$\mathbf{v}_j$ 都是一个 $d_{\text{head}}$ 维浮点向量——$b$ 字节/参数时，单个 token、单头、单层需要 $2 \times d_{\text{head}} \times b$ 字节。

**第 2 步：乘头数与层数。** 每层 $n_{\text{kv}}$ 个 KV 头（MHA = 全部头；GQA = 共享组数），每 token 每层 $2\,n_{\text{kv}} d_{\text{head}} b$ 字节；$L$ 层互不共享（各自有自己的 $W^K, W^V$），得每 token 总量 $2\,L\,n_{\text{kv}} d_{\text{head}} b = 2\,L\,d_{\text{kv}} b$。

**第 3 步：乘序列长度与 batch。** 缓存随生成线性累积：$\ell$ 个位置各存一份；每个并发请求各存各的（前缀不共享时）——总量即公式。**结论：缓存对 $\ell$、$B$ 线性，对架构参数（$L, d_{\text{kv}}, b$）一次性连乘。**

**第 4 步：MHA 实算**（7B 级：$L=32$、32 头 × 128 维 = $d_{\text{kv}} = 4096$、fp16 即 $b=2$）：

$$
\text{每 token} = 2 \times 32 \times 4096 \times 2\ \text{B} = 524{,}288\ \text{B} = 0.5\ \text{MB}
\qquad
128\text{K 上下文} = 0.5\ \text{MB} \times 131{,}072 = 64\ \text{GB}
$$

**第 5 步：GQA 实算**（32 查询头共享 8 组 KV：$n_{\text{kv}} = 8$，$d_{\text{kv}} = 8 \times 128 = 1024$，其余同上）：

$$
\text{每 token} = 2 \times 32 \times 1024 \times 2\ \text{B} = 128\ \text{KB} \;\;(\text{MHA 的 } 1/4)
\qquad
128\text{K}: 16\ \text{GB} \;\xrightarrow{\;\text{int8}(b{=}1)\;}\; 8\ \text{GB}
$$

与上文实算表逐项一致——同一公式，改的只是 $n_{\text{kv}}$ 与 $b$ 两个因子。

</details>

**几何/结构解释**（配图）：四个因子各管一个"放大维度"，GQA 砍的只是其中一环。

```mermaid
graph LR
    T["每 token 缓存"] --> F1["2 份<br/>（K 和 V）"]
    T --> F2["L 层<br/>（各层独立 KV 投影）"]
    T --> F3["d_kv 维<br/>（GQA：唯一可砍的因子）"]
    T --> F4["b 字节<br/>（fp16→int8 减半）"]
    F1 --> M["总量 = 四因子 × 序列长 × batch"]
    F2 --> M
    F3 --> M
    F4 --> M
```
...
- **batch 是隐形乘数**：公式对 $B$ 线性——10 路并发就是 10 倍缓存；"单测 128K 放得下"不代表"服务化放得下"；
- **GQA 砍到极限有代价**：MQA（$n_{\text{kv}}=1$）缓存最省但质量损失明显——$n_{\text{kv}}$ 是质量与显存的连续旋钮，不是免费开关；
- **量化缓存≠免费**：int8/int4 的 KV 量化带轻微精度损失，且与"权重量化"独立（一个动权重、一个动缓存，两本账）；
- **多 LoRA 服务会复制**：不同适配器若不共享基座缓存，缓存按适配器数翻倍——部署账本里最常被漏记的一项。

> 👉 **一句人话**：KV Cache 的账就一行乘法——"2 × 层数 × KV 维度 × 精度 × 长度 × batch"；要省显存，能动的只有 KV 维度（GQA/MQA）和精度（量化）两个因子，其余因子都是架构的物理常数。
...
**真实取舍一：Pre-Norm 的胜利是"训练稳定 > 理论表达力"的胜利，且胜负未盖棺。** Pre-Norm 牺牲了一点理论表达力换来"残差主路零关卡"，让深模型训练免惊吓——这是现代 LLM 清一色 Pre-Norm 的原因。但 Post-Norm 并未被判死：DeepNorm 类工作通过重新缩放残差分支，把 Post-Norm 训到千层量级。摆位问题的本质是"梯度通路"与"表示归一"放在哪条路上，两个目标都有解，只是成本不同。

**真实取舍二：深窄 vs 浅宽——参数相同，账单不同。** 同为参数量 $N \approx 12Ld^2$，每 token 计算量都约 $2N$（FLOPs 只认参数总量），但**推理延迟主要由层数 $L$ 决定**（$L$ 次必须串行的前向），**吞吐上限主要由宽度决定**（大矩阵乘更好吃满 GPU）。于是：深窄的 loss 常更低（更多串行组合逻辑）但延迟差；浅宽延迟好、并行友好但可能欠"深度"。选型不是看哪个"更先进"，而是看你的服务是延迟敏感还是吞吐敏感。

**真实取舍三：MoE 把"知识容量"的边际成本从算力搬到了显存。** 训练与推理的**算力**按激活参数计（便宜），**显存**按总参数计（昂贵）——所以开源 MoE 涌现的本质，是"用显存预算买知识容量"这条新供应链成立（呼应下一页 SOTA）。副作用：路由器成为新的单点故障源，负载均衡超参的调校成本被摊进每一次训练。

<details>
<summary>🧮 高级深问 3 则：FFN 为什么 4×、RMSNorm 为什么敢删减均值、词嵌入绑不绑定</summary>

**问 1：FFN 升维为什么是 4 倍？门控 FFN 为什么常写成 8d/3？** 4 倍是原始论文的经验选择，后被沿用成默认。现代门控 FFN（SwiGLU 类）从两个矩阵变三个（门控分支 + 值分支 + 降维），参数是 $3 \times d \times d_{\text{ff}}$；为与"两矩阵 4 倍升维"的 $8d^2$ 预算对齐，取 $d_{\text{ff}} = 8d/3$——即 $3 \times d \times \frac{8}{3}d = 8d^2$。**门控多花一个矩阵的钱，就从"升维 4 倍"换来了乘性非线性，值不值由消融实验说话，各家报告普遍说值。**

**问 2：RMSNorm 为什么敢把 LayerNorm 的"减均值"删掉？** LayerNorm 做两件事：中心化（减均值）+ 缩放不变（除标准差）。RMSNorm 的论点：起主要作用的是缩放不变性，中心化对逐 token 归一化贡献很小；删掉它少一次统计归约，大规模训练更快、稳定性不降。现代主流 LLM 基本都改用 RMSNorm——又一次"删掉理论修饰、保住工程功能"的案例。

**问 3：词嵌入矩阵（输入查表）和输出投影（logits 层）要不要共享参数（Tied Embedding）？** 共享可省一份 $\text{词表} \times d$（词表 15 万、$d=4096$ 时约 0.6B，不是小钱）；不共享的论点是输入端"查表"与输出端"打分"的最优子空间并不相同，大词表、多语言场景下分开训常略好。小模型省参数优先（共享），大模型边际参数不敏感（常不共享）——没有普适答案，只有预算答案。

</details>

## 🔬 SOTA 演进（2023–2026）

**长上下文工程**（把 4K 级模型拉到 128K~1M 级的两条主力路线）：

- **YaRN 类 NTK 感知缩放（一句话）**：位置插值把所有频率等比压缩回训练范围，短距离（高频）分辨率被不必要地牺牲；NTK 感知路线改为**高频外推、低频插值**——保住"相邻几个词"的精细位置感，只压缩"相隔很远"的粗粒度编码；YaRN 再叠加注意力温度修正。配合几百步量级的少量微调，就能把 RoPE 模型的可用上下文拉长数倍到数十倍（公开实践量级，经验参考）。
- **上下文并行（Context Parallelism，一句话）**：把**序列维度**切分到多卡（与数据并行/张量并行正交），每卡持有自己分片的 QKV，算注意力时以环形传递（Ring Attention 类）互相"借"对方分片的 KV——超长上下文的显存墙被转化为通信问题。百万级 token 的训练与推理全靠这层拼图（并行的完整谱系见[第 4 章分布式训练](../../3-深度学习/04-深度学习基础/04-分布式训练与模型规模.md)）。

## ❓ 开放问题

1. **名义上下文 vs 有效上下文**：大海捞针测试满分的模型，在多文档聚合推理这类"真用满长上下文"的任务上仍会掉分——"能检索到"不等于"能整合利用"，长上下文的评测学本身就是开放问题。
2. **位置编码的补丁何时到头**：外推、线性插值、NTK、YaRN……是同一根理论木桩上的一串补丁。位置信息的编码是否存在"从第一性原理推出"的最优形式，尚无答案。
3. **MoE 的均衡问题**：负载均衡辅助损失的系数调校近乎玄学；DeepSeek-V3 公开的**无辅助损失均衡**（直接在路由打分上加偏置项调控负载）是新尝试，其代价与边界仍在被社区评估——专家坍缩（多数专家得不到训练）在大规模下是否被彻底解决，未定。
